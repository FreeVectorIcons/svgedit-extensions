/* globals svgEditor */
import svgViewSourceDialogHTML from './svgViewSourceDialog.html';

const template = document.createElement('template');
template.innerHTML = svgViewSourceDialogHTML;

/**
 * @class SeSvgViewSourceDialog
 */
export class SeSvgViewSourceDialog extends HTMLElement {
  /**
  ...
  constructor() {
    ...
    this.$dialog = this._shadowRoot.querySelector('#se-svg-view-source-dialog');
    ...
    this.$copiedLabel = this._shadowRoot.querySelector('#copy-text');
    this.$codeEditor = this._shadowRoot.querySelector('#svg_view_source');
  }
  ...
  init(i18next) {
    ...
    this.setAttribute('tools-source_save', 'Copy');
  ...
  connectedCallback() {
    const onCancelHandler = () => {
      document
        .getElementById('se-svg-view-source-dialog')
        .setAttribute('dialog', 'close');
      document
        .getElementById('se-svg-view-source-dialog')
        .setAttribute('value', '');
    };
    const onCopyHandler = () => {};
    const onSaveHandler = () => {
      this.$sourceTxt.select();
      document.execCommand('copy');
      this.$copiedLabel.style = 'color: white; display: inline';
      setTimeout(() => (this.$copiedLabel.style = 'display: none'), 3000);
    };
...
// Register
customElements.define('se-svg-view-source-dialog', SeSvgViewSourceDialog);
